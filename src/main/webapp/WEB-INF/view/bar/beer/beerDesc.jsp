...
$().ready(function(){
	function loadEvals(scrollTop){
		...
		$.get("<c:url value="/api/evalRead/${desc.beerId}"/>",{},function(response){
	...
	function appendEvals(eval){
	      ...
	      var top = $("<span class = 'writer'>" + nickname +"</span><span class = 'regist-date'>"+eval.evalRegistDate+"</span>")
	      evalDiv.append(top);
	      
	      var body = $("<div class = 'body'>" + eval.beerEvalTalk +"</div>");
	      evalDiv.append(body);
		...
		<div class="row">
			...
			<div id = "beerEval">
				<c:forEach items="${beerEval }" var="beerEval">
					<div class ='eval'>
						<span class = 'writer'>${beerEval.userVO.nickname }</span>
						<span class = 'regist-date'>${beerEval.evalRegistDate }</span>
			...
			<div id = "createEval">
				<form id = "writeEvalForm">
					<div>
						<select id ="beerEvalPoint" name ="beerEvalPoint">
							<option>1</option>
							<option>2</option>
							<option>3</option>
							<option>4</option>							
					...
					<div>
						<input type ="button" id="writeEvalBtn" value = "등록"/>
					</div>
				</form>
			</div>
	<!-- evaluation -->
